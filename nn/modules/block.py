...
class MSConv(nn.Module):
    ...
    def forward(self, x):
        ...
        x_c = torch.stack([self.convs[i](x_group[..., i]) for i in range(len(self.convs))])
        x_c = rearrange(x_c, 'g bs ch h w -> bs (g ch) h w')
        x_c = self.conv_1x1(x_c)
        return x_c
class Bottleneck_MSConv(Bottleneck):
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
...
class MSC2f(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(Bottleneck_MSConv(self.c, self.c, shortcut, g, k=(3, 3), e=1.0) for _ in range(n))
#######################################################################
...
class HL_PAN(nn.Module):
    def __init__(self, in_planes, flag=True):
        super(HL_PAN, self).__init__()
